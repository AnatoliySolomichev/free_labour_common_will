...
const AxisLedgerRow* row_of(const std::vector<AxisLedgerRow>& v, const std::string& s) {
    for (const auto& r : v) if (r.slug == s) return &r;
    return nullptr;
}
...
TEST_F(AxisLedgerTest, SharesAreReadStraightOffTheDeals) {
    ...
    // Строки в каноническом порядке — по весу свидетельств.
    EXPECT_EQ(led.front().slug, "physical");             // 10.7 против 9.5 и 3.8
}
...
TEST_F(AxisLedgerTest, SpreadComesStraightFromWhatDealsSaid) {
    ...
    // Значения 0.1 / 0.5 / 0.9 при равных весах: σ = 0.32659...
    EXPECT_NEAR(d->spread, std::sqrt(0.32 / 3.0), 1e-6);
}
