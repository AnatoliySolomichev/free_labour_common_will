...
def run_sim(out_dir):
    ...
    # Базис — СЛЕДСТВИЕ УПОТРЕБЛЕНИЯ, а не список в коде (ИР-022): оси
    # ранжируются по часам работ, которые их несут, и берётся ровно столько,
    # сколько данные способны рассудить (kMinRowsPerColumn на столбец).
    # Свободного члена нет и долей нет: час, у которого все степени нули, — час,
    # в котором ничего не происходило.
    MIN_ROWS_PER_COLUMN = 10
    # Ранг — взвешенный РАЗБРОС оси по наблюдениям, не объём: ось, одинаковая у
    # всех, не различает ничего и вдобавок возвращает выброшенную константу.
    # Считается только по профилям, без взгляда на ставки.
    wtot = sum(o['hours'] for o in obs)
    s1, s2 = {}, {}
    for o in obs:
        for ax, v in cat[o['slug']]['axes'].items():
            s1[ax] = s1.get(ax, 0.0) + o['hours'] * v
            s2[ax] = s2.get(ax, 0.0) + o['hours'] * v * v
    spread = {a: max(0.0, s2[a] / wtot - (s1[a] / wtot) ** 2) for a in s1}
    room = len(obs) // MIN_ROWS_PER_COLUMN
    take = max(0, room - 1)                      # один столбец под кандидата
    names_a = sorted(sorted(spread, key=lambda a: (-spread[a], a))[:take])
    names_b = names_a + ['разряд']

    def feats(o, with_grade):
        ax = cat[o['slug']]['axes']
        x = [ax.get(a, 0.0) for a in names_a]
        if with_grade:
